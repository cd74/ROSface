...
from face.srv import FaceSrv,FaceSrvResponse
...
def imgprocess():
    ...
    cv2.imwrite('imgs/'+filename1,frame1)#save raw image
    print('process')
    str2 = ''
    if(flag=='1'):  #compare face
	    name = call.compare('imgs/'+filename1)
        #rt = ()
	    #if(name is not None):
	    ...
	    w,l,h,t = call.detect('imgs/'+filename1,frame1,flag) #detect face and save face image
	    w,l,h,t = str(w),str(l),str(h),str(t)
	    str2 = ','.join([w,l,h,t])
	    #if(w is not None):
	    return 'saved' if(w is not None) else 'noneface'
        #else:
            #return 'noneface'
...
s = rospy.Service('face', FaceSrv, handle)
